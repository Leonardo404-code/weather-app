...
export default function Search(){
    ...
    async function handleSearch(){
        const response = await api.get(`/weather?key=${key}&city+name=${input}`);
        ...
        return(
            <SafeAreaView style={styles.container}>
                ...
                colors={['#1ED6FF', '#97C1FF']}>
                    <Text style={styles.data}>{city.results.data}</Text>
    ...
    return(
        <SafeAreaView style={styles.container}>
            ...
            {error && <Text style={{marginTop: 25, fontSize: 18, color: '#FFF'}}>{error}</Text>}
